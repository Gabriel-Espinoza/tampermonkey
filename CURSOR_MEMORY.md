...
- Fuzzy corrections also append a **memo suffix** by default (`YNAB_DEFAULT_FUZZY_MEMO_SUFFIX` in `lib.js`, placeholders `{ynabDate}` / `{bankDate}`). YNAB flags have no text field; memo carries the explanation. `config.fuzzyDateMemoSuffix: ''` disables memo changes; omit key for default.
...
- **Cuenta corriente (mismo frame):** `scripts/santander/cuenta-corriente-movimientos.js` — hash `saldos/main/movimientos` (case-insensitive), `__santanderCcMovsInitDone`, consola `[Santander CC]`. Firma de tabla: cabecera con **saldo** además de fecha/detalle y monto cargo/abono (la TC en los dumps del repo no trae columna Saldo). Fechas desktop a menudo **DD/MM** sin año: `expandShortFechas` antes de `buildMovimientosWithImportIds` (listado reciente primero, tope “hoy”, y no filas posteriores a la fila previa). Sin heurística de cuotas. Hooks de iframe usan `data-ynab-santander-cc-iframe-hook` para no chocar con el módulo TC.
- **Loader Santander:** en `.../Private_new/frame/` el loader llama **dos** `init` (TC + CC) con `accountId` distintos (`santander.nacional` vs `santander.cc`); cada módulo solo actúa en su hash.
...
- **Saldo inicial:** fila detalle `SALDO INICIAL` se omite en `extractMovimientos` (no CSV / no YNAB); el arrastre de fecha sigue aplicando antes del `continue` para no romper filas siguientes.
